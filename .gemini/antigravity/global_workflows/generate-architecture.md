...
## Overview
This action generates a comprehensive fullstack architecture document that covers all technical aspects of the system design. The document follows the structured template defined in `./templates/arc42-architecture-tmpl.yaml` and provides detailed guidance for AI-driven development. It should accurately identify the right projects from knowledge/projects and put in `Solution Strategy & Project Modules.

**IMPORTANT**: Use existing projects as far as possible by analyzing knowledge/SUMMARY*.md. DO NOT CREATE NEW TECH STACK OR PROJECT UNLESS ABSOLUTELY NECESSARY.

**IMPORTANT**: Create or Amend a progress.md inside project directory for checkpointing the current progress.
...
- **Technical Preferences** (optional): Check all SUMMARY-***.md files inside directory `i2o-feature-ai-docs/knowledge/projects`, Check knowledge base articles in `i2o-feature-ai-docs/knowledge/kb_pdfs`, Check Bigquery schema definitions inside `i2o-feature-ai-docs/knowledge/bigquery_schemas`, Check PG schema definitions inside `i2o-feature-ai-docs/knowledge/pg_schema`.
