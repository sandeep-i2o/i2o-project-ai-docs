...
- Frontend: use `frontend-developer` for Angular/TypeScript/UI work in `frontendapplication-i2o`
- Backend: use `backend-developer` for Java service work
- Full-stack: run frontend and backend streams in parallel when both are required
- UI changes: always run Playwright MCP validation after implementation
...
5. Transition the story to `Selected for Development` using command below

$ acli jira workitem transition --key IAC-73 --status "Selected for Development" --yes
...
$ acli jira workitem transition --key IAC-73 --status "IN PROGRESS" --yes
