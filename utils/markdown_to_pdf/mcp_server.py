...
def render_mermaid(code: str, tmp_dir: Path, index: int) -> Path:
    ...
    if not shutil.which("mmdc"):
        # If mmdc is missing, we can't render but we shouldn't crash the whole thing
        # Maybe return a placeholder or just leave as is
        return src
...
@mcp.tool()
def convert_markdown_to_pdf(markdown_text: str, output_path: str) -> str:
    """
    ...
    with tempfile.TemporaryDirectory() as tmp:
        tmp_dir = Path(tmp)
        html = md_to_html(markdown_text, tmp_dir)
        HTML(string=html).write_pdf(str(output_path_obj))
        
    return f"PDF successfully created at {output_path}"

@mcp.tool()
def convert_markdown_file_to_pdf(input_path: str, output_path: Optional[str] = None) -> str:
    """
    Reads a Markdown file and converts it to a PDF.
    
    Args:
        input_path: The absolute path to the .md file.
        output_path: Optional. The absolute path for the output PDF. If not provided, 
                     it will be saved in the same directory as the input file with a .pdf extension.
    """
    ...
    if output_path is None:
        output_path_obj = input_path_obj.with_suffix(".pdf")
    else:
        output_path_obj = Path(output_path)
        
    markdown_text = input_path_obj.read_text(encoding="utf-8")
    return convert_markdown_to_pdf(markdown_text, str(output_path_obj))
