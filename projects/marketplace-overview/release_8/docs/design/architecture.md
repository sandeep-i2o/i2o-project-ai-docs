...
**Project:** BP-MO-001 | **Release:** release_8 | **Date:** 2026-03-04  
**Status:** Draft
...
- **Reuse existing stack**: Angular 15 frontend, Spring Boot 3.x backends, BigQuery data warehouse, PostgreSQL metadata DB.
...
└───────────────────┬────────────────┬────────────────────────────────┘
           ...
           └────────┬──────┘  └────────────────────────┘
                    │ BigQuery Client
           ┌────────▼──────────────────────────────────┐
           │   Google BigQuery (CC_I2O_DATA_MART)      │
           │  Source: viz_lost_sales,                  │
           │          reseller_progress_by_week,        │
           │          listings_progress_by_week,        │
           │          viz_reseller_filtered_detail,     │
           │          bp_wbr_detail                     │
           │  Sink:   marketplace_kpi_weekly_snapshot  │
           └───────────────────────────────────────────┘
           ...
           │  Weekly Cron → aggregation job → BigQuery  │
           └────────────────────────────────────────────┘
```
...
| KPI Aggregation Store | BigQuery view/table `marketplace_kpi_weekly_snapshot` (aggregated from `CC_I2O_DATA_MART` source tables) | BigQuery is existing analytics warehouse |
...
└── MarketplaceOverviewWeeklyAggregationTask.java
    ...
    # Writes to marketplace_kpi_weekly_snapshot BQ table
```
...
```
Browser                  i2o-reseller                              BigQuery
  │                           │                                        │
  │ POST /widget/getCardData  │                                        │
  │ { componentId: "bp_marketplace_overview_card",                     │
  │   filters: { org_id, brand_id, region, start_date, end_date } }   │
  │──────────────────────────>│                                        │
  │                           │── BQ: SELECT * FROM marketplace_kpi_weekly_snapshot
...
```
i2o-scheduler                                                BigQuery
  │  [Monday 06:00 UTC]                                          │
  │  MarketplaceOverviewWeeklyAggregationTask.run()              │
  │                                                              │
  │── SELECT Analytics KPIs ──────────────────────────────────────>>│
  │   FROM CC_I2O_DATA_MART.viz_lost_sales                         │
  │       (lost_sales, marketplace, region, period)                │
  │   FROM CC_I2O_DATA_MART.reseller_progress_by_week              │
  │       (closed_reseller_count, buy_box_winning_reseller_count)  │
  │   FROM CC_I2O_DATA_MART.listings_progress_by_week              │
  │       (List_Count_Closed, week_date)                           │
  │<── Analytics results per marketplace+brand+region ────────────│
  │                                                               │
  │── SELECT Brand Violation KPIs ────────────────────────────────>>│
  │   FROM CC_I2O_DATA_MART.viz_reseller_filtered_detail           │
  │       (brand_violation counts via enforcement_status CTE)      │
  │<── BV results ─────────────────────────────────────────────────│
  │                                                               │
  │── SELECT Enforcement Centre KPIs ─────────────────────────────>>│
  │   FROM CC_I2O_DATA_MART.bp_wbr_detail                          │
  │       (queue, cnd_pending, cnd_sent, test_purchase)            │
  │<── EC results ──────────────────────────────────────────────────│
  │                                                               │
  │── INSERT INTO marketplace_kpi_weekly_snapshot ─────────────────>>
  │   (org_id, week_start, marketplace, brand_id, region, kpi cols) │
  │<── success ─────────────────────────────────────────────────────│
  │                                                               │
  │── INSERT INTO marketplace_scheduler_audit_log ─────────────────>>
...
### 7.2 BigQuery Sink Table: `marketplace_kpi_weekly_snapshot`
...
CREATE TABLE `{project}.CC_I2O_DATA_MART.marketplace_kpi_weekly_snapshot` (
  ...
  org_id              STRING NOT NULL,        -- Tenant identifier (from Keycloak token)
  week_start          DATE NOT NULL,          -- Monday of the week
  week_end            DATE NOT NULL,          -- Sunday of the week
  marketplace         STRING NOT NULL,        -- AMAZON | WALMART | EBAY | TARGET
  brand_id            STRING NOT NULL,        -- Org brand identifier
  region              STRING NOT NULL,        -- US | UK | DE | etc.
...
-- Cross-tenant isolation: org_id is extracted from the Keycloak Bearer token at request time and injected into every BQ query by i2o-reseller. It is never trusted from the client request payload.
```
...
### 7.3 Key TypeScript Interfaces (Frontend)
...
| `CC_I2O_DATA_MART.marketplace_kpi_weekly_snapshot` | BigQuery | Read (query time) | Aggregated weekly destination table |
...
| `org_marketplace_config` | i2o-master-data (PostgreSQL) | Read | Activation status per org/marketplace |
...
| Scenario | Handling |
|----------|---------|
| BigQuery timeout | Return 504; frontend shows "Data temporarily unavailable" |
...
| Scheduler job failure | Retry up to 3 times with 30-minute backoff. After all retries exhausted, write `status='FAILED'` to `marketplace_scheduler_audit_log` and trigger PagerDuty alert. On-call team follows Section 17 runbook. |
...
```properties
# Marketplace Overview — BigQuery
marketplace.bq.dataset=marketplace_kpi_weekly_snapshot
marketplace.bq.project=${GCP_PROJECT_ID}
...
| Data isolation | `org_id` extracted from Keycloak token at request time; injected into all BQ queries as a mandatory `WHERE org_id=?` filter. `org_id` is never accepted from client request payload. Sink table `marketplace_kpi_weekly_snapshot` has `org_id` as the first clustering column. Cross-tenant isolation integration tests are required (see Section 14.2). |
...
| Concern | Strategy |
|---------|---------|
| Dashboard load time | BQ `marketplace_kpi_weekly_snapshot` is pre-aggregated weekly; query is simple SELECT with 3-4 filters → < 1s |
...
- Spring Boot `@SpringBootTest` for `MarketplaceOverviewController` with mock BigQuery and mock PostgreSQL
...
| 3 | Data | BigQuery pre-aggregation pattern consistent with platform | ✅ PASS | Follows existing scheduler+BQ pattern |
| 4 | Security | Tenant isolation enforced at data layer | ✅ PASS | `org_id` is mandatory column in `marketplace_kpi_weekly_snapshot` (Section 7.2); all BQ queries filter by `org_id` extracted from Keycloak token (Section 12); cross-tenant integration test required (Section 14.2). |
...
| 11 | Backend | Consistent with existing Spring Boot patterns | ✅ PASS | Controller/Service/DTO/BQ pattern as in i2o-reseller |
| 12 | Testing | Testing strategy covers unit + integration + E2E | ✅ PASS | Section 14; cross-tenant isolation test added (Section 14.2). |
| 13 | Performance | Pre-aggregation prevents slow BQ queries at request time | ✅ PASS | Weekly snapshot table + in-memory cache for metadata |
...
- **Secondary:** `data_staleness_flag = true` in the BQ snapshot table for the current week.
...
3. If transient (BQ quota, network): re-trigger task manually via scheduler admin endpoint `POST /admin/scheduler/run?task=MarketplaceOverviewWeeklyAggregationTask`.
...
Confirm by querying:
```sql
SELECT COUNT(*) FROM CC_I2O_DATA_MART.marketplace_kpi_weekly_snapshot
...
| WoW Change % | `wow_change_pct` | `marketplace_kpi_weekly_snapshot` (self-join) | `(current_week.lost_sales - prev_week.lost_sales) / NULLIF(prev_week.lost_sales, 0) * 100` | Percentage | NULL if no prior week exists; frontend renders `--` | Trial marketplaces (eBay, Target) |

---

*Generated: 2026-03-04 | Updated: 2026-03-04 (AR-001 through AR-007 resolved) | Release: release_8 | Template: arc42 (adapted)*
