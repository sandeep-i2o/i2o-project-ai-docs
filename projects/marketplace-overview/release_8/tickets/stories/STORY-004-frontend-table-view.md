---
id: STORY-004
title: "[MO] Frontend: Table View — AG Grid with KPI Columns"
...
# STORY-004 — Frontend: Table View — AG Grid with KPI Columns

## Context

The Table View is the alternative layout on the Marketplace Overview page, activated via the Card/Table toggle in the Filter Bar. It uses AG Grid Community (already licensed and configured in `frontendapplication-i2oretail`) to display all four marketplaces as rows with all 10 KPI columns plus a Status column. Data is loaded via `POST /report/getGridCardData` with `componentId: 'bp_marketplace_overview_table'`, `pageSize: 10`, `sort: 'marketplace ASC'`.
...
|--------|-------------|--------|-------|
| Marketplace | `marketplace` | Logo + name | Static images per marketplace |
...
| Status | `status` | ACTIVATED badge / Initiate Trial button | Trial: link to dialog (STORY-005) |
...
- AG Grid virtual scroll for performance.
...
```json
{
  ...
  "filters": { "marketplace": [...], "brand": [...], "region": "US", "start_date": "...", "end_date": "..." },
  "page": 0,
  "pageSize": 10,
...
2. All 12 columns (Marketplace, 10 KPIs, Status) are visible.
...
7. Filter changes (brand, region, week) trigger a fresh `getGridCardData` API call.
8. AG Grid virtual scroll is enabled.
9. Unit tests cover column definition and `null` rendering (≥ 70% component coverage).

## Definition of Done Checklist

- [ ] `MarketplaceTableComponent` created with correct AG Grid column definitions
...
- [ ] AG Grid virtual scroll configured
...
- Section 6.1 — Table view data load sequence
