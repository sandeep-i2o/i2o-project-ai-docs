...
- **Sink:** New PostgreSQL table `marketplace_kpi_weekly_snapshot` (indexed by `org_id, week_start, marketplace, brand`).
...
| Area | Deliverable |
|------|-------------|
| PostgreSQL sink DDL | `CREATE TABLE marketplace_kpi_weekly_snapshot` (org_id, week_start, marketplace, brand, region, 13 KPI cols) |
...
2. Task runs on Monday 06:00 UTC cron and writes all 13 KPI columns per `(org_id, marketplace, brand, region, week_start)`.
